...
public class OrderAchieveHelper {
	...
	public OrderAchieveHelper(ServletContext servletContext) {
	...
	public void updateOrderAchieve(Integer userId) {
		...
		if (archieveProgressSvc.getOneAchieveProgress(userId, achieveId) == null) {
			...
			archieveProgressSvc.addAchieveProgress(userId, achieveId, articles.size(), orders.size(),
					achieveVO.getOpenDate(), 13);
		}

		// 將目前的成就任務與使用者訂單成立的狀態，更新到成就進度DB
		// 注意 :userId 與 AchieveId是「複合主鍵」
		AchieveProgress userAchieveProgress = archieveProgressSvc.updateAchieveProgress(userId, achieveVO.getAchiId(),
				articles.size(), orders.size(), achieveVO.getOpenDate(), 13);
